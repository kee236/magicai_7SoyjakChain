...
class GeneratorController extends Controller
{
    ...
    public function buildChatStreamedOutput(Request $request)
    {
        ...
        if (PlanHelper::userPlanAiModel() && ! $request->get('chatbot_front_model')) {
            $default_ai_engine_new = AiModel::query()
                ->where('key', PlanHelper::userPlanAiModel())
                ->first();

            if ($default_ai_engine_new) {
                $chat_bot = PlanHelper::userPlanAiModel();
                $default_ai_engine = $default_ai_engine_new->ai_engine;
            }
        }
