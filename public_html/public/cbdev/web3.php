...
function bxc_eth_swap($amount, $cryptocurrency_code_from, $cryptocurrency_code_to = false, $address = false) {
    bxc_eth_load();
    if (!$address) {
        $address = bxc_settings_get_address('eth');
    }
    if (!$cryptocurrency_code_to) {
        $cryptocurrency_code_to = bxc_settings_get('eth-node-conversion-currency');
    }
    if (bxc_crypto_whitelist_invalid($address)) {
    ...
    if (!$is_eth) {
        array_unshift($path, bxc_eth_get_contract($cryptocurrency_code_from, $network)[0]);
    }
    $chain_id = bxc_web3_chain_id($is_mainnet ? 'eth' : 'sepolia');
    ...
    if (strpos($hash, '0x') !== 0) {
        return bxc_error($hash, 'bxc_eth_swap');
    }
...
function bxc_eth_transfer($amount, $cryptocurrency_code = 'eth', $to = false, $from = false, $wallet_key = false) {
    ...
    if (!$from)
        $from = bxc_settings_get_address('eth');
    if (!$to)
        $to = bxc_settings_get('eth-node-transfer-address');
    $to = trim($to);
    if (bxc_crypto_whitelist_invalid($to, false, $cryptocurrency_code))
        return 'whitelist-invalid';
    ...
    if (!$balance) {
        ...
        if (strpos($hash, '0x') !== 0)
            return bxc_error($hash, 'bxc_eth_transfer');
        bxc_eth_wait_confirmation($hash);
        $balance = bxc_eth_get_balance('eth', $from, 'wei');
    }
    ...
    $transaction = new Transaction([
        ...
        'data' => $data ? '0x' . $data : '',
        'chainId' => bxc_web3_chain_id($network == 'mainnet' ? 'eth' : 'sepolia')
    ]);
...
function bxc_eth_curl($method, $params = []) {
    return bxc_node_rpc('eth', $method, $params);
}
                ...
                if ($error) {
                    ...
                    if ($unit === 'hex') {
                        ...
                        if ($unit === 'dec')
                            ...
                            $balance = intval(Utils::toWei($balance, bxc_eth_decimals_to_name($contract_info[1]))->ToString());
                    }
