...
<header
    class="site-header @if ($fSectSettings->preheader_active) top-1 @else top-1 @endif group/header absolute inset-x-0 z-50 text-yellow transition-[background,shadow] [&.lqd-is-sticky]:bg-yellow [&.lqd-is-sticky]:text-black [&.lqd-is-sticky]:shadow-[0_4px_20px_rgba(0,0,0,0.03)]"
>
    <nav
    ...
    >
        ...
        <div
            class="site-nav-container basis-1/3 transition-all max-lg:absolute max-lg:right-0 max-lg:top-full max-lg:max-h-0 max-lg:w-full max-lg:overflow-hidden max-lg:bg-[#343C57] max-lg:text-yellow [&.lqd-is-active]:max-lg:max-h-[calc(100vh-150px)]">
                        ...
                        @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                            @if (in_array($localeCode, explode(',', $settings_two->languages)))
                                <a
                                    ...
                                    rel="alternate"
                                    hreflang="{{ $localeCode }}"
                                >{{ country2flag(substr($properties['regional'], strrpos($properties['regional'], '_') + 1)) }} {{ $properties['native'] }}</a>
                            @endif
                        @endforeach
                    </div>
                @endif
        ...
        <div class="flex basis-1/3 justify-end gap-2 max-lg:basis-2/3">
            ...
            @auth
                <div class="mx-3">
                    <x-button
                        link="{{ LaravelLocalization::localizeUrl(route('dashboard.index')) }}"
                        label="{!! __('Dashboard') !!}"
                        border="border-[2px] border-white !border-opacity-0"
                        ...
                        text="text-yellow group-[.lqd-is-sticky]/header:text-black group-[.lqd-is-sticky]/header:hover:text-yellow"
                    />
                </div>
            @else
                <x-button
                    link="{{ LaravelLocalization::localizeUrl(route('login')) }}"
                    label="{!! __($fSetting->sign_in) !!}"
                    type="outline"
                    border="border-[2px] border-white !border-opacity-10 group-[.lqd-is-sticky]/header:border-black"
                    text="text-yellow group-[.lqd-is-sticky]/header:text-black group-[.lqd-is-sticky]/header:hover:text-yellow"
                />
                <x-button
                    link="{{ LaravelLocalization::localizeUrl(route('register')) }}"
                    label="{!! __($fSetting->join_hub) !!}"
                    border="border-[2px] border-white !border-opacity-0"
                    bg="bg-white !bg-opacity-10 hover:!bg-opacity-100 group-[.lqd-is-sticky]/header:bg-black group-[.lqd-is-sticky]/header:hover:!bg-opacity-100"
                    text="text-yellow group-[.lqd-is-sticky]/header:text-black group-[.lqd-is-sticky]/header:hover:text-yellow"
                />
            ...
            <button
                class="mobile-nav-trigger group flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white !bg-opacity-10 group-[.lqd-is-sticky]/header:bg-black lg:hidden"
            >
...
@if ($app_is_demo)
    <div class="fixed bottom-10 start-8 z-50">
        <a
            class="inline-flex h-12 w-12 items-center justify-center rounded-full bg-[#262626] transition-all duration-300 hover:-translate-y-1 hover:scale-110 hover:shadow-md"
...
@if ($fSetting->floating_button_active)
    <div
        id="myBtn"
        @class([
            'fixed z-50 hidden bottom-8 max-md:!hidden',
            'start-24' => $app_is_demo,
            'start-8' => $app_is_not_demo,
        ])
    >
        <a
