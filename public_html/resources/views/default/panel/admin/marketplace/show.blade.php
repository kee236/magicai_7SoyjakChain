                ...
                <div class="flex w-full flex-col gap-8 lg:w-4/12 lg:ps-8">
                            ...
                            @endif
                                @if($item['licensed'] != true && $item['price'])
                                    <x-button
                                            ...
                                            variant="ghost-shadow"
                                            href="#">
                                        <x-tabler-shopping-cart id="{{ $item['id'].'-icon' }}" class="size-7 text-{{ in_array($item['id'], $cartExists) ? 'green' : 'gray' }}-500"/>
                                    </x-button>
                                @endif
                        </div>
                    ...
                    >
                        ...
                        <ul class="mx-auto mb-10 flex flex-col gap-2 text-start text-base font-semibold text-heading-foreground lg:w-11/12">
                            @foreach (['VIP Support', '5 Hours of Customization', 'Direct Access to Development Team', 'Proactive Issue Reporting', 'Unlimited Themes & Skins', 'Exclusive Roadmap Access', 'Priority Feature Requests', 'Access to All Extensions'] as $item)
...
@push('script')
    ...
    @includeWhen(\App\Helpers\Classes\Helper::showIntercomForBuyer(),'default.panel.layout.includes.buyer-intercom')
@endpush
